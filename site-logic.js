...
/**
 * Hash map for character responses to ensure O(1) retrieval performance.
 * This replaces inefficient if-else or switch statements as the character count grows.
 */
...
/**
 * Standardized helper for appending messages to the chat history.
 * Optimization: Uses appendChild and textContent for O(1) DOM updates, avoiding expensive innerHTML re-parsing.
 * @param {HTMLElement} history - The chat history container element.
 * @param {string} sender - The name of the sender (e.g., 'You' or character name).
...
function startChat(name, intro) {
    ...
    history.textContent = ""; // Clear existing history
...
function sendMessage() {
    ...
    // Optimization: Use hash map for O(1) response lookup instead of O(N) if-else chain
    setTimeout(() => {
