...
/**
 * Optimized character responses hash map for O(1) lookup.
 * This replaces inefficient if-else chains as the number of characters grows.
 */
const characterResponses = {
    ...
    'Chihiro': "I'm working at the bathhouse to save my parents. Have you seen any soot sprites?",
    'Haku': "I remember my name now, thanks to you. Be careful with Yubaba."
};

/**
 * Appends a message to the chat history using efficient DOM methods.
 * Performance optimization: Uses appendChild and textContent for O(1) updates,
 * avoiding the O(N^2) complexity and security risks of innerHTML.
...
function appendChatMessage(sender, text) {
    ...
    // textContent is faster than innerText/innerHTML and prevents XSS
...
function startChat(name, intro) {
    ...
    if (chatDisplay) chatDisplay.style.display = "block";
    if (chatName) chatName.textContent = "Chat with " + name;

    if (history) {
        // Clear history and add intro
        history.textContent = "";
