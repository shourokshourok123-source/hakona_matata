...
/**
 * Performance optimization: Global hash map for character responses to achieve O(1) retrieval complexity.
 * Replacing nested if-else/switch chains ensures consistent performance regardless of character count.
 */
const characterResponses = {
    ...
    'Chihiro': "I have to stay brave to save my parents. Everything here is so strange!",
    'Haku': "Remember your name, and you'll find your way back. I'll help you however I can."
};
...
function postReview(title) {
    ...
    // Performance optimization: achieve O(1) update by appending the new review directly to the DOM
    // instead of re-rendering the entire list from storage (O(N)).
...
/**
 * Helper to create a review element.
 * Uses textContent for O(1) performance and security against XSS.
 */
...
function displayReviews(title) {
    ...
    title = title.toUpperCase();
    const reviews = (JSON.parse(localStorage.getItem('reviews'))[title] || []);

    // Clear list and use fragment for efficient, batched DOM updates
    list.textContent = "";
...
/**
 * Standardized O(1) DOM update for chat messages.
 * Uses textContent and appendChild to avoid expensive innerHTML re-parsing and layout thrashing.
 */
...
function startChat(name, intro) {
    ...
    const history = document.getElementById('chat-history');
    if (history) history.textContent = ""; // Clear previous chat securely
...
function sendMessage() {
    ...
    // Performance optimization: O(1) DOM append for user message
    appendChatMessage("You", msg);

    input.value = "";

    // O(1) retrieval for bot response using global hash map
    setTimeout(() => {
