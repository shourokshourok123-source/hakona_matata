...
## 2024-05-27 - Critical Path Optimization
**Learning:** For static sites with heavy external assets (Pinterest, YouTube), resource hints like `preconnect` are vital. Preloading background images specified in CSS is crucial because browsers normally find them late in the waterfall. Using `fetchpriority="high"` on the primary LCP image (logo or hero) further reduces TTFB to LCP. Note: `dns-prefetch` is redundant when `preconnect` is used for the same domain.
**Action:** Apply `preconnect` for CDNs, `preload` for critical CSS-defined backgrounds, and `fetchpriority` for hero images. Avoid redundant `dns-prefetch`.
