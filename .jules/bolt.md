...
## 2026-07-06 - Lazy LocalStorage Initialization
**Learning:** Top-level synchronous `localStorage.setItem` calls during script load are a hidden performance tax on every page navigation. They block the main thread and trigger unnecessary disk I/O even when the data isn't needed for the current view.
**Action:** Always use lazy initialization with defensive fallbacks (e.g., `JSON.parse(localStorage.getItem('key') || '[]')`) inside the functions that consume the data, rather than global initialization blocks.
