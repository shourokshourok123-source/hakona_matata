...
## 2024-05-25 - O(1) Complexity for Conditional Logic
**Learning:** For features like character chat with multiple potential responders, using `if-else` or `switch` statements introduces O(N) complexity for message dispatch. While negligible for 2-3 characters, it becomes a bottleneck as the "cast" grows.
**Action:** Use a global hash map (object) for character data and responses to ensure O(1) retrieval regardless of how many characters are added to the application.
